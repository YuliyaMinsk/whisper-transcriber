...
# Устойчивый режим (если текст залип в "...")
python transcribe_diarize.py -s 2 --robust
...
### Устойчивый режим (`--robust`)

Иногда результат «залипает»: расшифровка прошла, а в тексте только точки (`...`).
Это классическое поведение Whisper на длинных записях: если запись начинается с
тихого/невнятного вступления (паузы, «эээ», «угу»), модель выдаёт на нём `...`, и
поскольку по умолчанию каждый следующий 30-секундный кусок получает предыдущий
текст как подсказку (`condition_on_previous_text=True`), эти `...` тащатся через
весь файл.

Флаг `--robust` отключает передачу предыдущего текста как подсказки
(`condition_on_previous_text=False`) — тогда тихий старт не портит остальную запись:

```bash
python transcribe_diarize.py -s 2 --robust
```

По умолчанию режим **выключен**: обычный путь Whisper, как правило, даёт лучшее
качество. Включай `--robust`, только если конкретная запись залипла в `...`.
